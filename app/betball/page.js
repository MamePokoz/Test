...
import { Coins, TrendingUp, Timer, Trophy, X } from 'lucide-react';
...
const FootballBettingSystem = () => {
  ...
  useEffect(()=>{
    ...
    const timer = setInterval(()=>setTimeLeft(prev => prev <= 1 ? 300 : prev-1),1000);
  ...
  return (
    <div style={{minHeight:'100vh',background:'linear-gradient(135deg,#0f2027,#203a43,#2c5364)',padding:'20px',fontFamily:'system-ui,sans-serif'}}>
      {/* Header */}
      <div style={{background:'linear-gradient(135deg,#667eea,#764ba2)',borderRadius:'20px',padding:'20px',marginBottom:'30px',boxShadow:'0 10px 30px rgba(0,0,0,0.5)'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap'}}>
          <h1 style={{margin:0,color:'white',fontSize:'2em'}}>⚽ FOOTBALL BETTING</h1>
          <div style={{display:'flex',alignItems:'center',gap:'15px',background:'rgba(0,0,0,0.2)',padding:'8px 12px',borderRadius:'12px'}}>
            <Timer size={20} color="white"/>
            <span style={{color:'white',fontWeight:'bold'}}>{formatTime(timeLeft)}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:'20px',marginBottom:'30px'}}>
        <div style={{background:'#1f1f2e',borderRadius:'15px',padding:'20px',color:'white',textAlign:'center',boxShadow:'0 8px 20px rgba(0,0,0,0.3)'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))',gap:'25px',marginBottom:'30px'}}>
        {matches.map(match=>(
          <div key={match.id} style={{background:'#1c1c2a',borderRadius:'15px',padding:'15px',color:'white',boxShadow:'0 5px 15px rgba(0,0,0,0.5)',transition:'0.3s'}}>
            <div style={{marginBottom:'10px',fontWeight:'bold',fontSize:'1.1em'}}>{match.teamA.flag} {match.teamA.name} vs {match.teamB.name} {match.teamB.flag}</div>
            <div style={{display:'flex',justifyContent:'space-between',gap:'10px'}}>
              {['home','draw','away'].map(choice=>(
                <div key={choice} style={{
                  background:bets[match.id]?.choice===choice ? choiceColors[choice] : '#2c2c3c',
                  color:bets[match.id]?.choice===choice ? '#fff' : '#ddd',
                  padding:'10px 0',
                  borderRadius:'10px',
                  textAlign:'center',
                  flex:'1',
                ...
                onClick={()=>handleBetChange(match.id,choice,bets[match.id]?.amount||0)}>
                  {choice.toUpperCase()} <br/> {choice==='home'?match.odds.home:choice==='draw'?match.odds.draw:match.odds.away}
                </div>
              ))}
            </div>
            <input type="number" placeholder="Amount" value={bets[match.id]?.amount||''} 
              onChange={e=>handleBetChange(match.id,bets[match.id]?.choice||'home',e.target.value)}
              style={{marginTop:'10px',width:'100%',padding:'10px',borderRadius:'10px',border:'none',outline:'none',textAlign:'center'}}
            />
          </div>
        ))}
      </div>
      ...
      <div style={{textAlign:'center',marginBottom:'30px'}}>
        <button onClick={handlePopup} style={{background:'linear-gradient(135deg,#ff6b6b,#ff8e53)',color:'white',border:'none',padding:'15px 25px',borderRadius:'15px',fontSize:'1.2em',cursor:'pointer',boxShadow:'0 5px 15px rgba(0,0,0,0.3)'}}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {lastResults.map((res,i)=>(
              <div key={i} style={{background:'#2c2c3c',padding:'10px',borderRadius:'10px'}}>
                <div style={{fontWeight:'bold',marginBottom:'5px'}}>
                  {res.match.teamA.name} vs {res.match.teamB.name}
                </div>
                <div>
                  Result: <span style={{color: res.won?'#4ade80':'#f87171'}}>{res.resultText}</span>
                </div>
                {res.bet && (
                  <div>
                    Your Bet: <span style={{color: {home:'#4ade80',draw:'#facc15',away:'#f87171'}[res.bet.choice]}}>
                      {res.bet.choice.toUpperCase()}
                    </span> {res.bet.amount} ฿ → Payout: {res.payout}
                  </div>
                )}
              </div>
            ))}
